...
# Take the first 25 molecules that have the lowest binding score
scores25 = pd.to_numeric(plot_df2[SCORE_COL], errors="coerce").dropna().head(25)

# Horizontal bars for readability with long names
plt.figure()
plt.barh(plot_df2[NAME_COL].astype(str), plot_df2[SCORE_COL])
plt.xlim(0, scores25.iloc[24] + 10)
plt.gca().invert_yaxis()
plt.xlabel("Binding score (lower = better)")
plt.title(f"Top 25 predicted binders for {uid}({fasta_symbol})")
plt.tight_layout()
plt.show()
